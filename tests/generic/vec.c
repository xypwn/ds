...
int main() {
	...
	int_vec_2d_fmt_register("%{IntVec}");
	...
	// 2D vector
	IntVec2D v2d = int_vec_2d();
	int_vec_2d_push(&v2d, int_vec());
	int_vec_2d_push(&v2d, int_vec());
	int_vec_push(&v2d[0], 1);
	int_vec_push(&v2d[0], 2);
	int_vec_push(&v2d[0], 3);
	int_vec_push(&v2d[0], 4);
	int_vec_push(&v2d[1], 69);
	int_vec_push(&v2d[1], 420);
	assert(v2d[1][1] == 420);
	fmts(buf, 4096, "%{IntVec2D}", v2d);
	assert(strcmp(buf, "{{1, 2, 3, 4}, {69, 420}}") == 0);
	int_vec_2d_del(v2d, 0);
	int_vec_2d_term(v2d);
